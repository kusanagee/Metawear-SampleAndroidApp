...
public class TemperatureFragment extends SingleDataSensorFragment {
    ...
    protected void setup() {
        Temperature.Sensor tempSource = tempModule.sensors()[selectedSourceIndex];
        if (tempSource.type() == SensorType.EXT_THERMISTOR) {
            ((Temperature.ExternalThermistor) tempModule.sensors()[selectedSourceIndex]).configure(gpioDataPin, gpioPulldownPin, activeHigh);
        }
        tempSource.addRoute(source -> source.stream((data, env) -> {
            ...
            return timerModule.schedule(TEMP_SAMPLE_PERIOD, false, tempSource::read);
        }).continueWithTask(task -> {
